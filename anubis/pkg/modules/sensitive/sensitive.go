...
func (m *Module) Run(cfg scanner.ScanConfig, findings chan<- scanner.Finding) error {
	...
	httpCfg.RateLimit = cfg.RateLimit / 2 // faster for file checks
	...
	for _, file := range eligible {
		...
		go func(f sensitiveFile) {
			...
			checkFile(client, baseURL, f, httpCfg, cfg, findings, m.Name())
		}(file)
	}
...
func checkFile(client *http.Client, baseURL string, f sensitiveFile, httpCfg utils.HTTPConfig, cfg scanner.ScanConfig, findings chan<- scanner.Finding, module string) {
	url := baseURL + f.path
