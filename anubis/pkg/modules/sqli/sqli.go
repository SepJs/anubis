...
func (m *Module) Run(cfg scanner.ScanConfig, findings chan<- scanner.Finding) error {
	...
	for _, param := range params {
		for _, payload := range testPayloads {
			if err := testParam(client, target, param, payload, httpCfg, cfg, findings, m.Name()); err != nil {
...
func testParam(
	...
	if err != nil {
		return nil // connection errors are expected with some payloads
	}
	...
	for _, sig := range errorSignatures {
		if strings.Contains(bodyLower, sig.pattern) {
			...
			return nil // one finding per param is enough for error-based
		}
	}
