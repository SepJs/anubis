...
func init() {
	...
	// Fixed: matches flags.go layout where reportLevel is an int
	rootCmd.Flags().IntVar(&reportLevel, "report-level", 1, "Detail level of the report")
	...
	// Fixed: matches flags.go layout where frameworkExamples and showRemediation are bools
	rootCmd.Flags().BoolVar(&frameworkExamples, "framework-examples", false, "Show exploit examples for detected frameworks")
	rootCmd.Flags().IntVar(&maxFrameworkExamples, "max-framework-examples", 3, "Maximum remediation examples displayed")
	rootCmd.Flags().BoolVar(&showRemediation, "show-remediation", true, "Show vulnerability remediation steps")
	...
	// Fixed: matches flags.go layout where modulePriority is a []string
	rootCmd.Flags().StringSliceVar(&modulePriority, "module-priority", []string{}, "Set module execution priority")
	...
	// Fixed: matches flags.go layout where externalAPI is a string (e.g. for API key values)
	rootCmd.Flags().StringVar(&externalAPI, "external-api", "", "External API key for OSINT integration")
	...
	// Intercept Cobra help to render custom structured layout
...
func printFullUsageExtended(cmd *cobra.Command) {
	...
	fmt.Printf("      --report-level           %-15s %s\n", "INT", white("Report depth level representation [Default: 1]"))
	...
	fmt.Printf("      --external-api           %-15s %s\n", "KEY", white("Integrate passive external intelligence streams via API key"))
