...
func (m *Module) Run(cfg scanner.ScanConfig, findings chan<- scanner.Finding) error {
	...
	httpCfg.RateLimit = cfg.RateLimit * 3 // more conservative rate for brute force
	...
	if cfg.Username != "" && cfg.Password != "" {
		testCredential(client, loginURL, cfg.Username, cfg.Password, httpCfg, cfg, findings, m.Name())
		return nil
	}
	...
	if cfg.AuthStrategy == "defaults" || cfg.AuthStrategy == "combined" {
		...
		for _, cred := range defaultCredentials {
			result := testCredential(client, loginURL, cred.user, cred.pass, httpCfg, cfg, findings, m.Name())
			if result == "blocked" {
				lockoutCount++
				if lockoutCount >= 3 {
...
func testCredential(
	...
	time.Sleep(time.Duration(httpCfg.RateLimit) * time.Millisecond)
