...
func (m *Module) Run(cfg scanner.ScanConfig, findings chan<- scanner.Finding) error {
	...
	// A single shared Limiter, guarded by a mutex, rather than one Limiter per
	// goroutine. Port scanning intentionally fans out wide (cfg.Threads*2), so
	// per-goroutine limiters would each pace independently and the *combined*
	// request rate would be threads× higher than --rate-limit implies. Sharing
	// one limiter keeps the aggregate pacing honest, at the cost of brief lock
	// contention — acceptable here since Wait() itself dominates the time spent.
