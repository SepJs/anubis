...
func outputFilename(base, ext string) string {
	...
	// Strip extension only from the filename part (not from directory components)
	// so "reports/anubis_target.com_20260618" doesn't get mangled.
	dir := filepath.Dir(base)
	file := filepath.Base(base)
	if idx := strings.LastIndex(file, "."); idx >= 0 {
		// Only strip if the extension is a known report format to avoid
		// stripping dots that are part of a hostname in the filename.
...
<footer>Anubis Security Scanner v1.0.0 — Vladimir Unknown — Authorized Use Only</footer>
</body>
</html>`
